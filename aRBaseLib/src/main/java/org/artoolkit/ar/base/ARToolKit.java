...
public class ARToolKit {
	...
	public boolean initialiseNative(String resourcesDirectoryPath) {
		...
		if (NativeInterface.arwChangeToResourcesDir(resourcesDirectoryPath)) {
			Log.i(TAG,"ARToolKit working directory is change to " + resourcesDirectoryPath);
		}
		else {
	...
	public void saveMaker(int uid,String path){
		NativeInterface.arwNftSaveMarker(uid,path);
	}

	public boolean tryGenNewMaker(byte[] image){
		return NativeInterface.arwNftTryCreateMarkerFromeImage(image);
	}

	public int genNewMaker(byte[] image){
		return NativeInterface.arwNftAddMarker(image);
	}
